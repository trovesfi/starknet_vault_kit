...
import {
  VesuExtendedMultiplierStrategy,
  VesuExtendedTestStrategies,
  VesuExtendedStrategySettings,
} from "@strkfarm/sdk";
...
const SIMPLE_SANITIZER = ContractAddr.from(
  "0x3798dc4f83fdfad199e5236e3656cf2fb79bc50c00504d0dd41522e0f042072"
);
const RELAYER =
...
//     const symbol = 'USDC'
//     const underlyingAsset = Global.getDefaultTokens().find(token => token.symbol === symbol)?.address!;
...
//     const vaultDeploymentInfo = await Deployer.prepareMultiDeployContracts(vaultContract, config, acc as any);
...
//     const redeemRequestDeploymentInfo = await Deployer.prepareMultiDeployContracts(redeemRequestContract, config, acc as any);
...
//     const vaultAllocatorDeploymentInfo = await Deployer.prepareMultiDeployContracts(vaultAllocatorContract, config, acc as any);
...
//     const managerDeploymentInfo = await Deployer.prepareMultiDeployContracts(managerContract, config, acc as any);
...
//deployAUMOracle("0x494bbd06d41526953e59c30c3bfa8ffbff38081ec49e2696c2627f0b058719")
/**
...
async function configureSettings(vaultContracts: VaultContracts) {
  ...
  const vaultContract = new Contract({
    abi: vaultCls.abi,
    address: vaultContracts.vault.toString(),
    providerOrAccount: provider as any,
  });

  const setRedeemContractCall = vaultContract.populate(
    "register_redeem_request",
    [vaultContracts.redeemRequest.address.toString()]
  );
  const setVaultAllocatorCall = vaultContract.populate(
    "register_vault_allocator",
    [vaultContracts.vaultAllocator.address.toString()]
  );

  const vaultAllocatorCls = await provider.getClassAt(
    vaultContracts.vaultAllocator.toString()
  );
  const vaultAllocatorContract = new Contract({
    abi: vaultAllocatorCls.abi,
    address: vaultContracts.vaultAllocator.toString(),
    providerOrAccount: provider as any,
  });

  const setManagerCall = vaultAllocatorContract.populate("set_manager", [
    vaultContracts.manager.address.toString(),
  ]);

  await Deployer.executeTransactions(
    [setRedeemContractCall, setVaultAllocatorCall, setManagerCall],
    acc as any,
    provider,
    "Setup vault configs"
  );
...
async function upgrade(
    ...
    const cls = await Deployer.myDeclare(contractName, packageName, config, acc as any);
    ...
    await Deployer.executeTransactions([call], acc as any, config.provider, 'Upgrade contract');
}
...
//     management_fees: uint256.bnToUint256(
//       Math.round(args.management_fees * 1e18)
//     ),
//     performance_fees: uint256.bnToUint256(
//       Math.round(args.performance_fees * 1e18)
//     ),
...
//   report_delay_seconds: number,
//   addedBufferPercent: number = 0.5
// ) {
//   const expectedReturnForTheReportDelayPercent =
//     (expectedAPYPercent * report_delay_seconds) / (365 * 24 * 60 * 60);
//   const output =
//     expectedReturnForTheReportDelayPercent +
//     expectedReturnForTheReportDelayPercent * addedBufferPercent;
//   console.log(
//     `Max delta for expected APY ${expectedAPYPercent}% and report delay ${report_delay_seconds} seconds is: ${output}%`
//   );
//   return output / 100;
// }

async function grantRole(vault: VesuExtendedMultiplierStrategy<VesuExtendedStrategySettings>, role: string, account: string) {
    const provider = config.provider;
    const cls = await provider.getClassAt("0x520a2e945dd0762e5284fc1b012f62ca04e238e105eb362d5e0ce208928729d");
    const contract = new Contract({abi: cls.abi, address:"0x520a2e945dd0762e5284fc1b012f62ca04e238e105eb362d5e0ce208928729d", providerOrAccount: provider as any});
    const grantRoleCall = contract.populate('grant_role', [role, account]);
    await Deployer.executeTransactions([grantRoleCall], acc as any, provider, 'Grant role');
}
...
//         ]
//     }], config, acc as any);
//     await Deployer.executeDeployCalls(calls, acc as any, provider);
// }

// async function deployLegacyToNewAvnuDecoderAndSanitizer() {
//     const provider = config.provider;
//     const calls = await Deployer.prepareMultiDeployContracts([{
//         contract_name: 'LegacyToNewDecoderAndSanitizer',
//         package_name: VAULT_ALLOCATOR_PACKAGE,
//         constructorData: []
//     }], config, acc as any);
//     await Deployer.executeDeployCalls(calls, acc as any, provider);
...
// async function setMinDelay(vault: VesuExtendedMultiplierStrategy<VesuExtendedStrategySettings>) {
//     const provider = config.provider;
//     const cls = await provider.getClassAt(vault.address.toString());
//     const contract = new Contract({abi: cls.abi, address: vault.address.toString(), providerOrAccount: provider as any});
//     // ! set the value
//     const setMinDelayCall = contract.populate('set_report_delay', [1]);
//     await Deployer.executeTransactions([setMinDelayCall], acc as any, provider, 'Set min delay');
// }
...
async function setConfig() {
  ...
  await setManagerRoot(vaultStrategy, ContractAddr.from(acc.address));
  // console.log("Granting role to", acc.address as string);
  //await grantRole(vaultStrategy, hash.getSelectorFromName('ORACLE_ROLE'), acc.address as string);
...
// })))
// deploySanitizer();
//upgrade('VaultAllocator', VAULT_ALLOCATOR_PACKAGE, '0x037d96fd7e05a4186e2900c3b330f816E4Ba6b8DDc94BaB8Af00AFaaf73910D2');
